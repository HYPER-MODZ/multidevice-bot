	...
	switch (command) {
            ...
            case prefix+'menu': case prefix+'help':{
                ...
                if (isGroup) {
                    ...
                    sendButton('location', from, `මචන් හායි ${pushname} 👋, me *${botName}*\n\nමෙම bot Beta *Multi-Device* Whatsapp වේ. ඔබ යම් ආකාරයක දෝෂයක් හෝ දෝෂයක් සොයා ගන්නේ නම්, කරුණාකර පළමුව තේරුම් ගන්න, හරිද?😖, අවශ්‍ය හෝ හදිසි නම් අයිතිකරු වාර්තා කරන්න🙏` + '\n\n' + ind.listMenu(time, salam, pushname, prefix), buttons)
               } else {
                   textImg(`මචන් හායි ${pushname} 👋, me *${botName}*\n\nමෙම bot Beta *Multi-Device* Whatsapp වේ. \nඔබ යම් ආකාරයක දෝෂයක් හෝ දෝෂයක් සොයා ගන්නේ නම්, කරුණාකර පළමුව තේරුම් ගන්න, හරිද?😖, අවශ්‍ය හෝ හදිසි නම් අයිතිකරු වාර්තා කරන්න🙏\n\nබොට් විශේෂාංග ලැයිස්තුවක් බැලීමට *${prefix}allmenu* ටයිප් කරන්න`)
                }
            }
            break
            case prefix+'allmenu': {
            ...
            case prefix+'leave':
                ...
                if (!isGroupAdmins) return reply(ind.adminOnly())
                if (!isBotGroupAdmins) return reply(ind.botNotAdmin())
                reply('Good Bye~ 👋').then(async res => await chika.groupLeave(from))
            break
		...
		break
           ...
           case prefix+'ig': case prefix+'igdl': case prefix+'instagram': case prefix+'instagramdl':
                ...
                xfar.Instagram(args[1]).then(async data => {
                    ...
                    txt += `*කරුණාකර මොහොතක් රැඳී සිටින්න, භාරදීමේ ක්‍රියාවලියේදී...*`
...
chika.sendMessage(from, anu, text, {quoted: msg})
break
case prefix+'xvideo':
case prefix+'xv':
query = args.join(" ")
...
chika.sendMessage(from, vid, video, {quoted: msg})
break
